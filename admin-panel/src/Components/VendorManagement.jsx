...
const VendorManagement = ({ vendorData, onSubmit }) => {
  ...
  const onFinish = async (values) => {
    ...
    try {
      let response;
      const endpoint = `http://localhost:4000/api/editVendor/${values.vendorId}`; // Use vendorId in the URL
      ...
      if (isEditing && values.vendorId) {
        ...
        response = await axios.post("http://localhost:4000/api/addVendor", requestData);
