...
const Restaurants = () => {
  ...
  useEffect(() => {
    const fetchRestaurants = async () => {
      try {
        const response = await axios.get("http://localhost:4000/api/getRestaurant"); // Update with your backend URL
  ...
  const handleTagClick = async (restaurant) => {
    ...
    try {
      await axios.put(`http://localhost:4000/api/updateRestaurant/${restaurant._id}`, {
      ...
      <div className="restaurants-grid">
        {restaurants.map((restaurant, index) => (
          ...
          >
            ...
            <img
              src={`http://localhost:4000${restaurant.imgPath}`}
